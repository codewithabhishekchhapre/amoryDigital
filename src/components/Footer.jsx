...
function Footer() {
  ...
  const handleSubmit = (e) => {
    axios.post("/data", formData).then(res => console.log(res))
    e.preventDefault();
    if (captchaVerified) {
      setShowModal(true);
    } else {
    ...
    <>
      <footer className="py-10 px-14 mt-4 bg-[url('/src/assets/hero-bg-3.png')] bg-cover bg-no-repeat bg-center text-white font-neueMachina">
        ...
        <div className="flex flex-col sm:flex-row gap-6 mt-6 items-center">
          ...
          <button
            ...
            className={`py-4 px-6 rounded-lg text-lg flex items-center gap-2 font-bold transition-all duration-300 ${captchaVerified ? "bg-white text-black" : "bg-gray-700 text-gray-500 cursor-not-allowed"
              }`}
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 px-4 sm:px-0">
          <div className="bg-gray-900 p-6 rounded-xl shadow-xl w-full sm:w-3/4 md:w-1/2 text-white max-h-[80vh] overflow-y-auto">
